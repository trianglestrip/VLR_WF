...
CUDA_DEVICE_FUNCTION CUDA_INLINE float testVisibility(
    ...
    float3 direction = make_float3(dirToLight.x, dirToLight.y, dirToLight.z);

    // 阴影 payload：0=可见, 非零=遮挡
    uint32_t payload = 0;
    optixTrace(topGroup, origin, direction, tmin, tmax, 0.0f,
               /* visibilityMask */ 0xFF,
               RAY_FLAG_TERMINATE_ON_FIRST_HIT,
               /* sbtOffset */ 0, /* sbtStride */ 1, /* missSbtIndex */ 1,
               /* payload */ 1, 4, &payload);

    return (payload == 0) ? 1.0f : 0.0f;
...
extern "C" __global__ void wavefrontSampleLights(
    ...
    if (std::isfinite(lightSolidAnglePDF) && lightSolidAnglePDF > 1e-10f)
    ...
    for (int i = 0; i < NumSpectralSamples && isFinite; ++i)
        isFinite = isFinite && isfinite(contrib.values[i]);
