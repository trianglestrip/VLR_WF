...
#### 5.4 材质特化 Kernel (4 天)
- [ ] 创建漫反射特化 Kernel
  - [ ] `wavefrontSampleBSDF_Diffuse()`
- [ ] 创建光滑反射特化 Kernel
  - [ ] `wavefrontSampleBSDF_Glossy()`
- [ ] 创建镜面反射特化 Kernel
  - [ ] `wavefrontSampleBSDF_Specular()`
- [ ] 实现动态 Kernel 选择
- [ ] **预期收益**: 15-25% 性能提升
- [ ] **验收**: 性能对比报告

**阶段 5 里程碑**: ✅ 性能达到预期目标（1.5-3x 加速）
...
- 完成阶段 1: 基础架构搭建
- 开始阶段 2: 核心 Kernel 实现
- 实现前 3 个 Kernel（GenerateRays, TraceRays, ProcessHits）
